...
public class AfterconditionDAO {
	// 싱글톤
	private static AfterconditionDAO dao = null;
	private AfterconditionDAO() {}
	public static AfterconditionDAO getInstance() {
		if( dao == null ) {
			dao = new AfterconditionDAO();
		}
		return dao;
	}
	...
	public List<AfterconditionDTO> selectList(
			Connection conn){
		String sql = " select distinct " + 
				...
				" join std_info si            on   sd.subj_d_code = si.subj_d_code " + 
		...
		ResultSet rs = null;
		ArrayList<AfterconditionDTO> list = new ArrayList<>();
		try {
			pstmt = conn.prepareStatement(sql);
			rs = pstmt.executeQuery();
			AfterconditionDTO dto = null;
			while(rs.next()) {
				dto =  new AfterconditionDTO();
