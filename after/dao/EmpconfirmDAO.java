...
public class EmpconfirmDAO {
	// 싱글톤
	private static EmpconfirmDAO dao = null;
	private EmpconfirmDAO() {}
	public static EmpconfirmDAO getInstance() {
		if( dao == null ) {
			dao = new EmpconfirmDAO();
		}
		return dao;
	}
	...
	public List<EmpconfirmDTO> selectList(
			Connection conn){
		...
		ResultSet rs = null;
		ArrayList<EmpconfirmDTO> list = new ArrayList<>();
		try {
			pstmt = conn.prepareStatement(sql);
			rs = pstmt.executeQuery();
			EmpconfirmDTO dto = null;
			while(rs.next()) {
				dto =  new EmpconfirmDTO();
