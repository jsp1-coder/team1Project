...
public class Answer_Set_DAO {
	
	private static Answer_Set_DAO dao = null;
	private Answer_Set_DAO() {}
	public static Answer_Set_DAO getInstance() {
		if( dao == null ) {
			dao = new Answer_Set_DAO();
		}
		return dao;
	}
	
	
	public ArrayList<Answer_Set_DTO> selectList(
		...
		ArrayList<Answer_Set_DTO> list = new ArrayList<>();
		try {
			pstmt = conn.prepareStatement(sql);
			pstmt.setString(1, ans_subj_d_code);
			rs = pstmt.executeQuery();
			Answer_Set_DTO dto = null;
			while(rs.next()) {
				dto =  new Answer_Set_DTO();
